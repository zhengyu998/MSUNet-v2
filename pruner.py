...
class Pruner_mixed(object):
    def __init__(self, model, start_epoch, end_epoch):
        self.model = model
        #self.mask_dict, self.sparsity_dict = self._get_mask_dict()
        self.start_epoch, self.end_epoch = start_epoch, end_epoch

        self.sparsity_1x1, self.sparsity_dw = self.get_sparsity_dicts(0.9, 0.5)
        self.pruner_list = [Pruner(model, start_epoch, end_epoch, self.sparsity_1x1),
    ...
    def get_sparsity_dicts(self, s_se=0.9, s_dw=0.5):
        sparsity_1x1 = {'blocks.2.0.se.conv_reduce.weight': s_se,
        ...
        # Todo: The first dw-conv was not included.
        ...
        return(sparsity_1x1, sparsity_dw)
